...
|Set-AzSKUserPreference|This command contains different switch params to set user preferences for AzSK commands. E.g. 1. OutputFolderPath to override default path 2. DoNotOpenOutputFolder to not open output folder by default 3. StoreComplianceSummaryInUserSubscriptions to keep track of latest scan results which would help to calculate subscription compliance % using Get-AzSKInfo command. |NA|
