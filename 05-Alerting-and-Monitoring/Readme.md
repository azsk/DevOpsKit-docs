...
- [Components of the Log Analytics workspace]
- [Setting up the Log Analytics workspace (Step by Step)]
- [Guide to Log queries]
...
This is achieved by the use of Log Analytics workspaces available in the Azure Portal.
...
### Components of the Log Analytics workspace
The AzSK Log Analytics workspace is used by the DevOps team for monitoring and 
generating a dashboard for security monitoring and alerting based on AzSK control evaluation events.

The out of box security dashboard generated by the AzSK Log Analytics workspace includes:
1. An "Overview tile" for the Log Analytics workspace summary
...
### Setting up the AzSK Log Analytics solution (Step by Step)
This section will walk you through the step-by-step experience of setting up the AzSK Log Analytics solution.

Note that for the Log Analytics workspace to display anything, you have to configure AzSK in SDL or CICD or
...
The AzSK Log Analytics setup involves two sets of steps - one set to be run by the monitoring team that owns 
...
then, ideally, the same workspace should be used for setting up the AzSK monitoring workspace as well. 
...
**Step-2 (Ops Team):** Capture the Workspace ID and Primary Key for the Log Analytics workspace by navigating to "Settings -> Advanced Settings -> Connected Sources -> Windows Servers".
