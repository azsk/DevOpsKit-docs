...
- [Guide to Logs queries]
- [Next Steps](Readme.md#next-steps)
- [Appendix](Readme.md#appendix)
  - [Creating a Log Analytics workspace]
  - [Testing Log Analytics connectivity](Readme.md#b-testing-oms-connectivity)
  - [Routing AzSK events to Log Analytics workspace]
  - [Leveraging other Monitoring Solutions from the Solutions Gallery](Readme.md#d-leveraging-other-oms-solutions-from-the-solutions-gallery)

--------------------------
### Overview: 
The Alerting & Monitoring features of AzSK empower DevOps teams with the following capabilities:
- a single pane of glass view of cloud security across DevOps stages
...
Out of the box, these capabilities can be leveraged via the Log Analytics Monitoring solution in AzSK.

However, a DevOps team can equally easily leverage a different system for log analytics (e.g., Splunk) and view the AzSK control evaluation events
...
### Components of the AzSK Monitoring Solution
The AzSK Monitoring Solution is deployed to a Log Analytics workspace that is used by the DevOps team for monitoring and 
...
CA mode to send events to the target Log Analytics workspace as mentioned in the "Routing AzSK events to Log Analytics..." section in [Appendix](Readme.md#appendix).
...
portfolio of applications for a business unit and (2) a central subscription that hosts the Log Analytics workspace for that business unit. We call the former subscriptions the 'app subscriptions' and 
...
involved. Indeed, most DevOps teams will have Dev rotating to support Ops and vice versa.)


**Step-1 (Ops Team): Deploy the AzSK Monitoring Solution**

**[1-a]** Use Set-AzureRmContext to choose the subscription corresponding to the Log Analytics workspace and run the command below 
...
**[1-b]** Obtain the workspaceId and sharedKey for the Log Analytics workspace you'd like to use for monitoring.
...
the DevOps stages (developer machines (SDL stage), your build environment (CICD stage) and operational environment (CA)) 
...
different DevOps stages. For instance, the view shown below can be generated by querying for AzSK control events 
corresponding to different DevOps stages ("SDL", "CICD", "CA") in respective blades. 
...
Go to https://docs.microsoft.com/en-in/azure/azure-monitor/learn/quick-create-workspace and follow the simple steps to create a new Log Analytics workspace.
...
(independently) sent to Log Analytics. Depending on the DevOps stage the mechanism used to 'wire up' AzSK with Log Analytics is different. 
...
2. In the build/deployment (CICD) stage, Log Analytics settings are specified via input parameters for the [AzSK SVTs build/release extension](../03-Security-In-CICD/Readme.md).

3. For Continuous Assurance (CA), the Log Analytics workspace info is specified in the input parameters of the [Continuous Assurance setup script](../04-Continous-Assurance/Readme.md).  
...
It is also unique in its coverage of events across DevOps stages.
...
In the next steps we will configure AzSK to send data to the Log Analytics workspace from a PowerShell session. This can be done by running commands discussed in [[B] Testing Log Analytics connectivity](Readme.md#b-testing-oms-connectivity).
