...
**Step-1: Setup**  
1. Install latest AzSK.AzureDevOps module.
...
Out of the box, these capabilities can be leveraged via the Log Analytics-based Monitoring solution in AzSK.AzureDevOps.

However, a dev ops team can equally easily leverage a different system for log analytics (e.g., Splunk) and view the AzSK.AzureDevOps control evaluation events in the alternate system. This can be accomplished by using connectors for Event Hubs or Webhooks in the AzSK.AzureDevOps.


### Components of the AzSK.AzureDevOps Monitoring Solution
The AzSK.AzureDevOps Monitoring Solution is deployed to a Log Analytics workspace that is used by the dev ops team for monitoring and generating a dashboard for security monitoring.

### Setting up the AzSK.AzureDevOps Monitoring Solution (Step by Step)
This section will walk you through the step-by-step experience of setting up the AzSK.AzureDevops Monitoring Solution.
...
b) you have setup AzSK.AzureDevOps installed to send events to that workspace or ADOSecurity scanner added to CICD pipeline.

### Step-1 (Ops Team): Deploy the AzSK.AzureDevOps Monitoring Solution
...
You should see data about AzSK.AzureDevOps events as query results. (Again, this assumes that by now AzSK.AzureDevOps 
...
### [B] Enabling Log Anaytics workspace connectivity into AzSK.AzureDevOps and routing AzSK.AzureDevOps events to Log Analytics ###
Let us look at how to send events to the Log Analytics workspace from AzSK.AzureDevOps running on a local machine. This is a handy way to 
test connectivity and to see if the <Log Analytics workspace> Logs can display the received events.

**Step-1 (App Team):** 
Connect the local (dev box) installation of AzSK.AzureDevOps to your Log Analytics workspace for sending AzSK.AzureDevOps control evaluation events.

Run the below in a PS session (this assumes that you have the latest AzSK.AzureDevOps installed).
```PowerShell
...
After this, all AzSK.AzureDevOps cmdlets, SVTs, etc. run on the local machine will start sending events (outcomes of 
...
**Step-2 (App Team):** Generate some AzSK.AzureDevOps events and validate that they are reaching the configured Log Analytics workspace.

Run a few AzSK.AzureDevOps cmdlets to generate events for the Log Analytics repo. 
...
AzSK.AzureDevOps events similar to the below ("_CL" stands for "custom log"):
...
```Powershell
#Setting attestation host project and auto bug logging for organization controls
Get-AzSKAzureDevOpsSecurityStatus -OrganizationName "<OrganizationName>" -AttestationHostProjectName "<Host Project Name>" -AutoBugLog All
```
...
The details for all these bugs can be found in *BugSummary.json* file that is stored in the folder : *%LOCALAPPDATA%\Microsoft\AzSK.AzureDevOpsLogs\Org_[yourOrganizationName]*. The information conveyed from this JSON looks as follows:
