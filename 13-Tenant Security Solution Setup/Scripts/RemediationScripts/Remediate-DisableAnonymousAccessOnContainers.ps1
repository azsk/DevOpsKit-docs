...
function RollBack-AnonymousAccessOnContainers
{
    ...
    switch ($RollBackType) 
    {
        ...
        {  
            ...
            $totalResourceToRemediate = ($resourceContext | Measure-Object).Count
            Write-Host "Found [$($totalResourceToRemediate)] storage account to perform roll back operation."
            ...
            try{
                if($totalResourceToRemediate -gt 0)
                {
                    $resourceContext | ForEach-Object{
                        $flag = $true
                        $context = $_.context;
